...
const GenerateThumbnail = ({setImage, setImageStorageId, image, imagePrompt, setImagePrompt}: GenerateThumbnailProps) => {
  ...
  const handleImage = async (blob: Blob, fileName: string) => {
    ...
    try {
      ...
      console.log(error)
      toast({
        title: "Ошибка генерации картинки",
        variant: 'destructive'
      })
    }
  }
  ...
  return (
    <>
      <div className="generate_thumbnail">
        ...
        >
          Использовать ИИ для создания картинки
        </Button>
        <Button
          type="button"
          variant="plain"
      ...
      {isAiThumbnail ? (
        <div className="flex flex-col gap-5">
          <div className="mt-5 flex flex-col gap-2.5">
            <Label className="text-16 font-bold text-white-1">
              ИИ для генерации изображения
            </Label>
            <Textarea
              className="input-class font-light focus-visible:ring-orange-1"
              placeholder="Предоставьте текст для создания картинки"
              rows={5}
              value={imagePrompt}
        ...
        <div className="image_div" onClick={() => imageRef?.current?.click()}>
          ...
          <div className="flex flex-col items-center gap-1">
            <h2 className="text-12 font-bold text-orange-1">Нажмите, чтобы загрузить</h2>
            <p className="text-12 font-normal text-gray-1">SVG, PNG, JPG, или GIF (макс. 1080x1080)</p>
          </div>
        </div>
      )}
      {image && (
