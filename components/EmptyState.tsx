...
const EmptyState = ({title, search, buttonLink, buttonText }: EmptyStateProps) => {
  return (
    <section className='flex-center size-full flex-col gap-3'>
        ...
        <div className='flex-center w-full max-w-[380px] flex-col gap-3'>
            <h1 className='text-16 text-center font-medium text-white-1'>{title}</h1>
            {search && (
                <p className='text-16 text-center font-medium text-white-2'>Попробуйте настроить поиск, чтобы найти то, что вы ищете.</p>
            )}
            {buttonLink && (
